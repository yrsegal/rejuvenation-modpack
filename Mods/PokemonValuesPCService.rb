...
class PokemonValuesPCService
  ...
  def lesserPositiveColor(darkWindow)
    return shadowc3tag(ColorArrays[:Default][0], Color.new(136,96,72)) if darkWindow
    return shadowc3tag(LightColorArrays[:Default][0], Color.new(123,36,28))
  end

  def lesserNegativeColor(darkWindow)
    return shadowc3tag(ColorArrays[:Default][0], Color.new(64,120,152)) if darkWindow
    return shadowc3tag(LightColorArrays[:Default][0], Color.new(27,79,114))
  end
  ...
  def tweaking(pkmn)
    ...
    while command >= 0
      ...
      command=Kernel.pbMessageAdvanced(_INTL("Tweak which?"), commands, -1, nil, command)
  ...
  def ivs(pkmn, origstats)
    ...
    while command >= 0
      ...
      command=Kernel.pbMessageAdvanced(_INTL("Change which IV?"), commands, -1, nil, command)
      summarywindow.dispose
      if command == 6
        for i in 0...6
          pkmn.iv[i] = 31
  ...
  def evs(pkmn, origstats)
    ...
    while command >= 0
      ...
      command=Kernel.pbMessageAdvanced(_INTL("Change which EV? (Total: {1}, max. {3}{2}</c3></o>)",
  ...
  def checkUnlocks
    ...
    if !$game_screen.pokemonvaluespc_unlocked_nature && $PokemonBag.pbQuantity(naturePower) > 0
      ServicePCList.exclaimSound
        wait(5)
