...
ModCacheInjection.hook(:trainertypes) {
  $cache.trainertypes[:CHATOTGIRL] = TrainerData.new(:CHATOTGIRL, {
    ID: "VulgarityEnthusiast",
...
ModCacheInjection.hook(:trainers) {
  $cache.trainers[:CHATOTGIRL] = {} unless $cache.trainers[:CHATOTGIRL]
  $cache.trainers[:CHATOTGIRL]["Girl"] = {} unless $cache.trainers[:CHATOTGIRL]["Girl"]
  $cache.trainers[:CHATOTGIRL]["Girl"][0] = TeamData.new(0, ["Girl", :CHATOTGIRL, 0], {
    :teamid => ["Girl", :CHATOTGIRL, 0],
    :mons => [
      {
        species: :CHATOT,
        level: 80,
...
class PokeBattle_Move
  ...
  def chatotcresteffect(attacker, opponent, hitnum, showanimation=true)
    if isSoundBased? && attacker.crested == :CHATOT
      ...
      if !opponent.isFainted?
        if ((opponent.ability != :SHIELDDUST && opponent.ability != :OBLIVIOUS) || opponent.moldbroken)
          ...
          when :DAMN
            if opponent.pbCanPetrify?(false)
              @battle.pbAnimation(:DECIMATION, attacker, opponent, hitnum) 
          ...
          when :BITCH
            if opponent.effects[:Taunt]<=0 && 
             ...
             (opponent.ability != :OBLIVIOUS || opponent.moldbroken)
              ...
              @battle.pbAnimation(:TAUNT, attacker, opponent, hitnum)
          ...
          if @move == :CHATTER
            if !opponent.effects[:Torment] && 
             (@battle.pbCheckSideAbility(:AROMAVEIL,opponent).nil? || opponent.moldbroken)
              @battle.pbAnimation(:TORMENT, attacker, opponent, hitnum)
  ...
  alias :chatotcrest_old_pbEffectTarget :pbEffectTarget
  def pbEffectTarget(attacker, opponent, hitnum = 0, alltargets = nil)

    dmg = chatotcrest_old_pbEffectTarget(attacker, opponent, hitnum, alltargets)

    chatotcresteffect(attacker, opponent, hitnum)
