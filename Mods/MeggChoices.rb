    ...
    if choices
      insertAfter(choices) {
        show_choices("(Do you want to choose which Pokémon you'll get?)") {
          ...
          default_choice("Random") {}
          choice("Choice") {
            text "(Which Pokémon do you want?)\\ch[#{Variables[:Route3MysteryEgg]},0,Azurill,Impidimp,Nacli,Togepi,Sneasel,Dhelmise,A-Litwick,Axew,Mienfoo,Pawniard,Trapinch,Hippopotas,Cottonee,Darumaka,Hatenna,Starly,Tyrunt,Larvesta,Mareanie,Stufful,Rookidee]"
          }
        }
      }
    end
    ...
    if choices
      insertAfter(choices) {
        show_choices("(Do you want to choose which Pokémon you'll get?)") {
          default_choice("Random") {}
          choice("Choice") {
            text "(Which Pokémon do you want?)\\ch[#{Variables[:AevianMysteryEgg]},0,A-Magikarp,A-Budew,A-Wimpod,A-Shroomish,A-Larvesta,A-Bronzor,A-Feebas,A-Sigilyph,A-Jangmo-o]"
