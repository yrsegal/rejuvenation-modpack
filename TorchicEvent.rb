...
InjectionHelper.defineMapPatch(203) { |map| # Pokestar Studios Interiors
  ...
  rawev.pages[1].trigger = 3 # autorun
  rawev.pages[1].list = InjectionHelper.parseEventCommands(
    [:Script, "$game_map.data[86,9,2] = 0"], # Deletes the torchic
    :EraseEvent,
    :Done) 

  rawev.pages[0].condition.variable_valid = true
  rawev.pages[0].condition.variable_id = Variables[:QuestXenogene]
  rawev.pages[0].condition.variable_value = 72
