...
class PokemonDataBox
  ...
  alias :bbui_old_refresh :refresh
  def refresh(loopstop = false)
    bbui_old_refresh(loopstop)
    pbShowStatsBoosts if loopstop == false
    pbShowTypeIcons if loopstop == false
  end
  ...
  def pbShouldShowTypeIcons?
    return false if @battler.isbossmon
    return true
  end


  def pbShowTypeIcons
    return nil if !pbShouldShowTypeIcons?

    pbUpdateTypeIconsBitmap if pbShouldUpdateTypeIconsBitmap?
    pbDrawTypeIconsTab
  end

  def pbShouldUpdateTypeIconsBitmap?
    return true if !defined?(@@typeIconData)
    return true if !@@typeIconData
    return true if !@@typeIconData[:battlers][@battler.index]
    return true if !@@typeIconData[:battlers][@battler.index][:bitmap]
    return true if @@typeIconData[:battlers][@battler.index][:bitmap].disposed?

    return true if @@typeIconData[:battlers][@battler.index][:types] != @battler.types(excludeReflector: true)

    return false
  end

  def pbGetTypeIconsPositionCoordinatesMapping
    return [
      [32, 0],
      [32, 40]
    ]
  ...
  def pbUpdateTypeIconsBitmap
    if !defined?(@@typeIconData) || !@@typeIconData
      @@typeIconData = {
        'battlers': []
      }
    end

    unless @@typeIconData[:battlers][@battler.index]
      @@typeIconData[:battlers][@battler.index] = {
        'stats': []
      }
    end
    @@typeIconData[:battlers][@battler.index][:types] = @battler.types(excludeReflector: true)
    @@typeIconData[:battlers][@battler.index][:bitmap] = pbBuildTypeIconsBitmap
  end

  def pbBuildTypeIconsBitmap
    pbEnsureTypeIconsGraphicsLoaded
    bitmap = @@statsBoostsData[:background].bitmap
    result = Bitmap.new(bitmap.rect.width * 2, bitmap.rect.height)
    result.blt(0, 0, bitmap, bitmap.rect)
    coordsMapping = pbGetTypeIconsPositionCoordinatesMapping
    types = @@typeIconData[:battlers][@battler.index][:types]
    for pos in 0...2
      next if types.length <= pos

      val = types[pos]
      next if !@@typeIconData[:types][val]

      x, y = coordsMapping[pos]
      bmp = @@typeIconData[:types][val].bitmap
      result.blt(x, y, bmp, Rect.new(0, 0, 32, 32))
    end
    return result
  end

  def pbEnsureTypeIconsGraphicsLoaded
    if !@@typeIconData[:background] || @@typeIconData[:background].disposed?
      @@typeIconData[:background] = AnimatedBitmap.new('Graphics/Pictures/Battle/typediamonds.png')
    end
    @@typeIconData[:types] = {} if !@@typeIconData[:types]
    typesToBeLoaded = []
    for type in $cache.types.keys
      next if @@typeIconData[:types][type] && !@@typeIconData[:types][type].disposed?
      typesToBeLoaded.push(type)
    end
    return nil if typesToBeLoaded.length <= 0

    for i in typesToBeLoaded
      @@typeIconData[:types][i] = AnimatedBitmap.new("Graphics/Icons/bosstype#{i}.png")
    end
  end

  def pbDrawTypeIconsTab
    statsBitmap = @@typeIconData[:battlers][@battler.index][:bitmap]
...
# show stat boosts (use builtin)
# show types with diamonds
# show effectiveness and field boosts
# select menu
