begin
  missing = ['0000.injection.rb', '0000.textures.rb'].select { |f| !File.exist?(File.join(File.dirname(__FILE__), f)) }
...
InjectionHelper.defineMapPatch(315) { |map| # Rose Theatre
  InjectionHelper.createNewEvent(map, 4, 25, "Larvesta Egg") { |event|
    event.newPage { |page|
